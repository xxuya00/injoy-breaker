    ...
    <section>
      ...
      <div>
        {dayData.items.map((item) => {
          ...
          const isSpecial = item.type === 'locked-until' || item.type === 'final';
          let sub = item.sub;
          if (item.type === 'locked-until' && !open) sub = '저녁 집회에서 함께 열립니다';
          if (item.type === 'final' && !open) sub = '앞의 자물쇠를 모두 깬 뒤 열립니다';
          return (
            <div
              key={item.id}
              className={`${styles.lock} ${open ? styles.lockOpen : styles.lockLocked} ${
                !isSpecial ? styles.tapable : ''
              }`}
            ...
            >
              ...
              <div className={styles.body}>
                ...
                <div className={styles.sub}>{open ? '✓ 열림' : sub}</div>
              </div>
