...
export const MED_TYPES: Record<string, TypeMeta> = {
  R_W: { name: '말씀 저널러', desc: '당신은 정해진 시간에 말씀을 펴고, 떠오르는 생각을 손으로 적어 내려가며 정리하는 사람이에요. 규칙성과 기록이 만나 꾸준함의 힘을 만들어내요.' },
  R_M: { name: '고요한 묵상가', desc: '당신은 정해진 시간, 조용한 공간에서 말씀을 마음에 깊이 새기는 사람이에요. 기록보다는 그 순간의 침묵과 여운을 소중히 여겨요.' },
  S_W: { name: '떠오르는 대로 적는 자', desc: '당신은 특별히 정해두지 않아도 말씀이 떠오를 때마다 짧게라도 기록하며 삶 속에서 은혜를 붙잡는 사람이에요.' },
  S_M: { name: '일상 속 묵상러', desc: '당신은 출퇴근길, 걷는 길, 잠들기 전... 일상 곳곳에서 자연스럽게 말씀을 떠올리고 마음에 담아두는 사람이에요.' },
};

export const PRAY_TYPES: Record<string, TypeMeta> = {
  R_V: { name: '기도의 용사', desc: '당신은 정해진 시간을 지켜 소리 내어 기도하는, 뜨겁고 꾸준한 기도의 사람이에요.' },
  R_Si: { name: '골방의 사람', desc: '당신은 정해진 시간, 조용한 자리에서 마음으로 하나님과 깊이 교제하는 사람이에요. 마태복음 6장의 ‘골방 기도’와 닮아있어요.' },
  S_V: { name: '언제나 기도쟁이', desc: '당신은 때와 장소를 가리지 않고, 마음이 움직일 때마다 소리 내어 기도로 반응하는 사람이에요.' },
  S_Si: { name: '마음의 기도자', desc: '당신은 정해진 형식 없이, 일상 속에서 조용히 마음으로 하나님께 아뢰는 사람이에요.' },
};
